...
from tensorflow.keras.preprocessing.image import (
    img_to_array,
)  # Can use cv2 or other libraries.
import cv2
...
face_classifier = cv2.CascadeClassifier(
    "haarcascades_models/haarcascade_frontalface_default.xml"
)
...
emotion_interpreter = tf.lite.Interpreter(
    model_path="emotion_detection_model_100epochs_opt.tflite"
)
...
while True:
    ...
    for x, y, w, h in faces:
        ...
        emotion_preds = emotion_interpreter.get_tensor(
            emotion_output_details[0]["index"]
        )
        ...
        roi_color = np.array(roi_color).reshape(
            -1, 200, 200, 3
        )  # input shape is (1, 200,200,3)
